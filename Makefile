...
## Dev - Clear the build and assets
clear:
...
build-subdir: export WEBPACK_PUBLIC_PATH = /rix/build
build-subdir: export ROUTER_DEFAULT_URI = http://localhost:8001/rix
build-subdir: clear build
	rm -rf public/resized
	symfony console cache:clear
	ulimit -S -n 2048 && symfony console stenope:build build/rix

## Serve the static version of the site from a subdir / with base url
serve-static-subdir:
	open http://localhost:8001/rix
